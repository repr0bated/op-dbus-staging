...
use op_dbus::native::rtnetlink_helpers::{
    add_default_route, add_ipv4_address, del_default_route, flush_addresses, link_up,
};
...
async fn find_active_interface() -> Result<(String, Ipv4Addr, u8, Ipv4Addr)> {
    ...
    let ifname =
        default_ifname.context("No default route found - cannot determine active interface")?;
    ...
    let prefix =
        prefix_len.context(format!("No prefix length found for interface '{}'", ifname))?;
...
async fn main() -> Result<()> {
    ...
    println!(
        "✓ Found active interface: {} with IP {}/{} and gateway {}",
    ...
    println!(
        "🔨 Creating OVS bridge {} with uplink port {} and internal port {}...",
    ...
    ovsdb
        .add_port(bridge_name, &uplink_ifname)
        .await
    ...
    add_ipv4_address(internal_port_name, &ip_addr.to_string(), prefix_len)
        .await
    ...
    add_default_route(internal_port_name, &gateway.to_string())
        .await
